    ...
    \subsection{Flow Law}
        ...
        \begin{equation}
            \dot{\epsilon}=A\left(\frac{\sigma}{\mu}\right)^2
                ...
                \right)
        \end{equation}
        Fitting data to the law is very sensitive to changes in the exponents. Typically data do not extend to low enough strains rates to constrain p and q \citep[e.g.][]{evans1979temperature} and these parameters are often assumed to both have values of 1 as suggested by theory \citep{frost1982deformation}. In this case data at strainrates of $10^{-8}$ and $10^{-9}$ 1/s do constrain the exponents (see supplement figure \ref{fig:CH2s_Fitcompare-moredata}) so we were able to invert for parameters on all data from 75°C to 550°C. The best fit is a constant Peierls stress ($\tau_p$) of $2.42\pm0.04$ GPa inner exponent p of 1, outer exponent q of $1.18\pm0.04$, an average activation enthalpy of $86\pm1.4$ kJ/mol, and a pre-exponential constant A of exp($-.62\pm0.1$). Additionally some pressure dependence of antigorite strength exists, but it is modest so it is not included here \citep{Proctor2016}.
