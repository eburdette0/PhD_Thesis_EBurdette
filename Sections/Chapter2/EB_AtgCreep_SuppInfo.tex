...
\begin{figure}
  ...
  \caption{
  a) Comparison of normalized stress in this work to data of \citet{brantut2012micromechanics}. b) Data recorded during the creep test on Westerly granite. Points denote measured strain rate after sample have settled to a constant rate. Acoustic emission (AE) data are counts/hits of events recorded during the experiment.
  }
  \label{fig:Westerly_Supplement}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/Creep Plot 9-20 comparison Annotatedv0.2.pdf}
  \caption{
  Compilation of other high pressure experimental antigorite deformation results plotted over results form the current study. Both cores \citep{hirauchi2020semi,Chernak2010,Escartin1997} and gouge \citep{Chernak2010,Proctor2016,Okazaki2016} are plotted.
  }
  \label{fig:Data_Comparison_supplement}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/Keishi_W15190914.jpg}
  \caption{Thin section cross-polarized image of undeformed mesh texture. Compression of samples was in the vertical direction.
  }
  \label{fig:ThinSection_Supplement}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/All_cutopen.pdf}
  \caption{Images of antigorite core macrostructure. light/white portions of samples have shear microcracks.
  }
  \label{fig:Macrostructure_Supplement}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/W2424_zoom005.pdf}
  \caption{Gentle curvature of sheets recovered from 550°C sample.
  }
  \label{fig:550C_Kinks_Supplement}
\end{figure}


\subsection{Mechanical Data Calculations}
    ...
    \begin{equation}
        x_{sample} = x_{LVDT2} - \frac{\sigma_{1, external}}{k_{lower_column}}
    \end{equation}
    ...
    To calculate strain rates during post-processing, we used the first derivative of a first order Savitzky-Golay filter (moving line fit) over a moving time window chosen for each step. For strain rates above $10^{-6} 1/s$, the window could be as short as 100 seconds, while the lowest strain rates require 10000 seconds. Strain rates chosen for flow law fits were the final point that was not influence by a disturbance or loading to the next stress. Wherever strain rate is plotted as a continuous function of time (S6-7), a single window length was chosen over the whole plot to best display data. 
    

\subsection{Hardening During Stress Steps}

    It is important to note that within strain steps samples clearly harden over time (S\ref{fig:Load-Time-Fast}). This is expected to some extent from other descriptions/results of brittle creep and high temperature creep tests where primary, secondary, and tertiary creep phases can be identified. For westerly granite calibration tests (S\ref{fig:Westerly_Supplement}), at each step samples clearly reach constant strain rates after relatively small strains (~0.2\%). Post-processing of antigorite data shows that it may continue to harden over similar 0.2\% strain steps (depicted in data for W2447-200 (S\ref{fig:Load-Time-Fast})). This decay is not obvious in live displays of data because it requires more processing power to smooth and compute than our rig control PC has available. 
    
    % The observed rate decay appears to be linear when plotted as either log(strain rate) vs. strain (S\ref{fig:Strainrate-Strain}) and log(strain rate) vs. time (S\ref{fig:Strainrate-Time}). The strain rate decreases by approximately a factor of 2 over 0.2\% strain as can be observed in the included strain-time plots for W2447-200 (S\ref{fig:Load-Time-Fast}), and the decay during hydraulic oil confined experiment W2521-75 (S\ref{fig:Strainrate-Strain}). (Data is FIR decimated by a factor of 100 to 0.1Hz for these plots).

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/W2447_StrainrateStrainStress.pdf}
  \caption{Experiment data from 200°C experiment. black->stress; red->strainrate; cyan->strain (log scale).
  }
...
\begin{figure}
  ...
  \caption{Load and Displacement plotted against time to show strain rate evolution during a stress step. Data is taken from the last portion of the curve. Rate changes by approximately a factor of 2.
  }
  \label{fig:Load-Time-Fast}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/W2526_720MPa_LVDT2_trace_wlines.pdf}
  \caption{Load and Displacement plotted against time to show strain rate evolution during a stress step. For "low" stresses, the displacement resolution limits determination of strain rate 
  }
  \label{fig:Load-Time-Slow}
\end{figure}
...
\begin{figure}
  ...
  \caption{Hardening of antigorite at room temperature where rate change and stress are plotted against strain. Decay appears to be approximately log-linear.
  }
  \label{fig:Strainrate-Strain}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/W2521_stress_strainrate_vs_time.pdf}
  \caption{Hardening of antigorite at room temperature where rate change and stress are plotted against time.
  }
  \label{fig:Strainrate-Time}
\end{figure}



\subsection{Uncertainty introduced due to hardening}
    The ratio of initial/final strain seems to be similar at most strains. This makes ratio/increment (hardening slope) much larger at small strain. Hardening literature plots the change as $d\sigma/d\epsilon$ vs. $\sigma$ \citep{kocks2003physics}. It's unclear if that's appropriate for materials with pressure-dependent strength. 
    
    To assess the potential uncertainty introduced into the flow law by choosing final instead of initial strain rates, we re-plotted the fit for initial strain rates. Because change is a similar factor of ~2 for most steps, the fit exponent and Peirels stresses don’t change significantly while pre-exponential and activation energy change by ~5\%. An example with the same exponents and peierls stress is plotted in Figure S10.

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/Final-Initial Fit Comparison.png}
  \caption{Comparison of Peirels' fits of Final (Blue, lower), and initial (orange, higher) strain rates at each stress step.}
  \label{fig:Strainrate-Time}
\end{figure}

\subsection{Flow Law Fit Methodology}
    A Peierls' flow law was fit to stress vs. log(strainrate) using a Markov-chain Monte-Carlo (MCMC-NUTS) optimizer due to the small local discontinuities in the fit. Results with errors are presented in Table SXX1 and figure S\ref{fig:KDE_plot}.
    \begin{table}[ht]
    \begin{tabular}{lllll}
    \hline
    ...
    \begin{figure}
      \includegraphics[width=\textwidth]{Figures/MCMC_KDE_Matrix1.png}
      \caption{KDE plot of parameter density for the MCMC fit.}
      \label{fig:KDE_plot}
    \end{figure}
    
    Low stress data have a larger influence on the extrapolated fit because they define the transition between stress and temperature activation dominated regions of the Peierls law. The start of this transition can be seen in the slight curvature of the fit around points at 480°C in figure \ref{fig:Mechanical_combo}. In the flow law, the region is defined by exponents p, q, and normalization factor $\tau_p$.
    Changes in the curvature have a very large effect on extrapolated viscosity. In this case  uncertainty in extrapolated viscosity decreases by 5 orders of magnitude when data at low stress is added to the fit (figure S\ref{fig:Fitcompare-moredata}).
    

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/AtgVisc_12-2-20wpeierls_comparison_1to1.pdf}
  \caption{Comparison of fits for fixed exponent p,q combinations before and after slow data is added to the dataset (for illustration). Slow strain rate data has a very strong effect on extrapolated viscosity.}
  \label{fig:Fitcompare-moredata}
\end{figure}

\subsection{Griggs Rig Friction/Accuracy Measurement}
    ...
    To further test the ability to match the results of high temperature assemblies to room temperature assemblies, we loaded fused silica at 700°C in a standard, mitre-ring assembly and in a hydraulically confined assembly at low pressure (S12). The results compare well, with the elastic modulus of the fused silica at high temperature matching the elastic modulus at low temperature. This indicates that we can compare all assemblies used in this study to each other and older results at similar conditions.

\begin{figure}
  \includegraphics[width=\textwidth]{Figures/Proposal_WaxCombo_small.pdf}
  \caption{Tests comparing old mitre ring seals to new carbide bushing seal friction during run-in through paraffin wax, and during deformation in contact with the grade 360 brass. Improvement in both static friction and rate dependent friction is 3-5x. Viscosity of run-in or “hit” material affects friction determination (lower zoomed rate steps).
  }
  \label{fig:Brass-Wax}
\end{figure}
...
\begin{figure}
  ...
  \caption{Elastic tests of fused silica at room temperature in hydraulic oil and a solid salt assembly at 700°C with traditional packing rings. Despite the differences in friction and hit quality, load-displacement data match at high loads indicating correct calibration of our load and displacement transducers in both new and old assemblies.
  }
  \label{fig:WC-FS_elastic}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{Figures/Thermal Model with TC_bp1.pdf}
    \caption{Thermal model results. Methods are detailed in Moarefvand et al (in press))}
    \label{fig:ThermalModel}
\end{figure}


\begin{table} %\begin{table}
\label{CreepFitPoints}
\centering
